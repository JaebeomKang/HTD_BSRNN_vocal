...
from .BSRNN import BandSplitModule, BandSequenceModelModule, MaskEstimationModule
...
class HTDemucs(nn.Module):
    ...
    def __init__(
        ...
        test_tensor = torch.rand(1,2,time_duration)
        test_frame = self._spec(test_tensor)
        self.frame_size = test_frame.shape[-1]
        del test_tensor, test_frame

        fc_dim = 22
        rnn_type = "LSTM"
        num_layers = 8

        # V8    -   TO MATCH TIME_BRANCH CH 384
        bandsplits = [
            (1000, 50),
            (4000, 50),
            (8000, 50),
        ...
        # # V8    -   TO MATCH TIME_BRANCH CH 192
        # bandsplits = [
        #     (1000, 25),
        #     (4000, 50),
        #     (8000, 80),
        ...
        self.splitmodule = BandSplitModule(
            sr=self.samplerate,
            n_fft=self.nfft,
            bandsplits=bandsplits,
            t_timesteps=self.frame_size,
            fc_dim=fc_dim,
            complex_as_channel=self.cac,
            is_mono=False,
        )

        self.bandsequence = BandSequenceModelModule(
            input_dim_size=fc_dim,
            hidden_dim_size=fc_dim*2,
            rnn_type=rnn_type,
            bidirectional=True,
    ...
    def forward(self, mix):
        ...
        z = torch.stft(mix.reshape(-1, length), n_fft=self.nfft, hop_length=self.hop_length, window=torch.hann_window(self.nfft).to(mix),
                        win_length=self.nfft, normalized=True, center=True, return_complex=True, pad_mode='reflect')
        _, freqs, frame = z.shape
        z = z.view(B, C, freqs, frame).contiguous()
        ...
        ## ENCODING FOR TIME BRANCH
        saved_t = []  # skip connections, time.
        lengths_t = []  # saved lengths for time branch.
        for idx, encode in enumerate(self.tencoder):
            inject = None
        ...
        ## ENCODING FOR FREQ. BRANCH
        x = self.splitmodule(x)
        x = self.bandsequence(x)
        ...
        for idx in range(len(self.tdecoder)):
            if idx >= offset:
        ...
        if length_pre_pad:
            x = x[..., :length_pre_pad]
        del xt, zout, z, std, mean, stdt, meant, lengths_t, saved_t, mix
        gc.collect()
